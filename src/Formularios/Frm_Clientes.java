...
public class Frm_Clientes extends javax.swing.JInternalFrame {
    ...
    public Frm_Clientes() {
        initComponents();
        CP = new Cls_Clientes();
        columnModel = jtb_cliente.getColumnModel();
        listar();
        iniciar();
    }

    private void listar() {
        jtb_cliente.setModel(CP.getDatosClientes());
    ...
    private void guardar() throws SQLException {
        ...
        if (num == 0) {
            ...
            if (txt_doc.getText().equals("")) {
                ...
                JOptionPane.showMessageDialog(null, "Debe ingresar el numero de telefono del cliente");
            } else if (direccion.equals("")) {
                JOptionPane.showMessageDialog(null, "Debe ingresar la direccion del cliente");
            } else if (email.equals("")) {
                JOptionPane.showMessageDialog(null, "Debe ingresar el correo electronico del cliente");
            } else {

                ResultSet result = CP.getCliente(doc);
                result.next();
                if (result.getInt(1) <= 0) {
                    
                    int respuesta = CP.registrarCliente(doc, nombre, direccion, telefono, email);
                    if (respuesta > 0) {
                    ...
                    JOptionPane.showMessageDialog(null, "¡El cliente ya se encuentra registrado, intentelo de nuevo!");
                }
            ...
            int respuesta = CP.actualizarCliente(doc2, nombre2, direccion2, telefono2, email2);
            if (respuesta > 0) {
                listar();
                limpiar();
                iniciar();
    ...
    private void initComponents() {
        ...
        jtb_cliente.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jtb_clienteMouseClicked(evt);
            }
        });
        ...
        jbt_guardar.setText("Registrar Cliente");
        ...
        jLabel7.setText("Llene la información, relacionada con el cliente a registrar.");
    ...
    private void jtb_clienteMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jtb_clienteMouseClicked
        bt_actualizar.setEnabled(true);
        bt_eliminar.setEnabled(true);
        int row = jtb_cliente.getSelectedRow();
        txt_doc.setText(jtb_cliente.getValueAt(row, 0).toString());
        txt_nom_cli.setText(jtb_cliente.getValueAt(row, 1).toString());
        txt_direccion.setText(jtb_cliente.getValueAt(row, 2).toString());
        txt_email.setText(jtb_cliente.getValueAt(row, 4).toString());
        txt_telefono.setText(jtb_cliente.getValueAt(row, 3).toString());

    }//GEN-LAST:event_jtb_clienteMouseClicked
    ...
    private void bt_eliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bt_eliminarActionPerformed
        int fila = jtb_cliente.getSelectedRowCount();
        if (fila < 1) {
            ...
            if (resp == 0) {
                if (CP.eliminarCliente(jtb_cliente.getValueAt(jtb_cliente.getSelectedRow(), 0).toString()) > 0) {
                    listar();
                    limpiar();
                    bt_eliminar.setEnabled(false);
                    bt_actualizar.setEnabled(false);
